...
```{r}
predict(model, newdata = data.frame(imdb_rating = 8.0, audience_rating = "Upright"), interval = "prediction")
```

For Rogue One we didn't do too badly. The model predicted an `audience_score` of 85.45% and the actual score was 88%. The lower and upper values are the 95% confidence interval for the score, meaning we are 95% confident that with an `imdb_rating` of 8.0, a `critics_score` of 85 and `audience_rating` of Upright, that the value of the `audience_score` will be between 71.79% and 99.11%.
